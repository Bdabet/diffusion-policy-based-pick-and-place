...
def main(output, robot_ip, vis_camera_idx, init_joints, frequency, command_latency, text_conditioning):
    ...
    with SharedMemoryManager() as shm_manager:
        with KeystrokeCounter() as key_counter, \
            ...
            STARTING_POS_1= [-7.10899248e-01, -2.56950736e-01,  6.30217522e-02,  2.87004895e+00, -1.19886725e+00, -2.42536075e-03]

            auto_record_running = False
            auto_record_request_stop = False
            auto_record_stage = 0
            deviated_initial_starting_pose = None
            timestamp = None
            intial_run = True
            gripper_state = 0 # start gripper in open state
            object_is_cylinder = True
            inital_starting_position = None
            # final_placing_position = POS_1

            
            NEW_SAMPLE_TOL = 0.01       # 10mm
            DEVIATION_VAL = 0.005       # 5mm
            FINAL_POS_VARIATION = 0.005 # 5mm
            START_POS_VARIATION = 0.01   # 10mm
            POS_REACHED_TOL = 0.001     # 1mm 
            ROT_REACHED_TOL = 0.001     # 1mm
            ...
            while not stop:
                try:
                    ...
                    if auto_record_running:
                        ...
                        if auto_record_stage == 0: # move downwards to object (only in first run) and determine final placing position
                            if intial_run:
                                target_pose[2] = 0.075 
                                intial_run = False
                                final_placing_pose = current_pose.copy()
                            # slightly vary the x,y part of final placing position
                            # always sample deviation starting from fixed initial final position
                            deviated_final_placing_pose = final_placing_pose.copy()
                            deviated_final_placing_pose[:2] += np.random.uniform(-FINAL_POS_VARIATION, FINAL_POS_VARIATION, size=2)
                        ...
                        if auto_record_stage == 1: # close gripper
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                action_array[6] = 1
                                auto_record_stage = 2
                                timestamp = time.monotonic()

                        if auto_record_stage == 2 and time.monotonic() - timestamp >= 0.5: # wait for a second to close gripper before lifting for movement
                            if np.linalg.norm(current_pose - target_pose) < POS_REACHED_TOL and current_gripper_state == 1:
                                target_pose[2] = 0.15
                                auto_record_stage = 3

            
                        if auto_record_stage == 3:  # sample new starting position and only move horizontally towards it
                                if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                    if np.any(inital_starting_position) == None:
                                        inital_starting_position = STARTING_POS_1.copy()
                                    while True:
                                        deviated_initial_starting_pose = current_pose.copy()
                                        deviated_initial_starting_pose[0] = inital_starting_position[0] + np.random.uniform(-FINAL_POS_VARIATION, FINAL_POS_VARIATION)
                                        deviated_initial_starting_pose[1] = inital_starting_position[1] + np.random.uniform(-FINAL_POS_VARIATION, FINAL_POS_VARIATION)
                                        # only advance stage if the target is valid
                                        if (np.abs(deviated_initial_starting_pose[0] - current_pose[0]) >= NEW_SAMPLE_TOL and
                                            np.abs(deviated_initial_starting_pose[1] - current_pose[1]) >= NEW_SAMPLE_TOL): # TOL = 0.01

                                            auto_record_stage = 4
                                            target_pose[:2] = deviated_initial_starting_pose[:2]
                                            # target_pose[2] = 0.08 # no need to move back downwards before episode starts
                                            print(f"Stage {auto_record_stage-1}: object in gripper, sampled new target {deviated_initial_starting_pose}")
                                            print(f"Entering stage {auto_record_stage}: Move to new target position")
                                            break
                                        else:
                                            print("Sampled starting pose too close, will resample in next cycle")


                        if auto_record_stage == 4: # sample and carry out random rotation
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL: # sampled  position reached

                                # sample random grasping angle
                                deviated_initial_starting_pose = current_pose.copy()
                                deviated_initial_starting_pose[2] = 0.075 # target should be below when object touches base
                                ...
                                print(f"Stage {auto_record_stage}: object in gripper, sampled new object rotation {deviated_initial_starting_pose}")
                        ...
                        if auto_record_stage == 5: # move back downwards before starting episode
                            if np.linalg.norm(current_pose - target_pose) < POS_REACHED_TOL: # sampled  position reached
                                target_pose[2] = 0.075
                                auto_record_stage = 6
                        
                        if auto_record_stage == 6: # lifting to horizontal moving height and start episode 
                        ...
                        if auto_record_stage == 7: # move horizontally to final placing position
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                print(f"stage {auto_record_stage}: moving to final position.")
                                target_pose[:2] = deviated_final_placing_pose[:2] # only need x,y coordinates
                                auto_record_stage = 8


                        if auto_record_stage == 8: # rotate to final orientation
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                print(f"stage {auto_record_stage}: rotating to final orientation.")
                                target_pose[3:] = deviated_final_placing_pose[3:]
                                auto_record_stage = 9

                        if auto_record_stage == 9: # move downwards 
                             if np.linalg.norm(current_pose[3:] - target_pose[3:]) < POS_REACHED_TOL:
                        ...
                        if auto_record_stage == 10: # open gripper 
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                action_array[6] = 0
                        ...
                        if auto_record_stage == 11: # time delay
