            ...
            while not stop:
                try:
                    ...
                    if auto_record_running:
                        ...
                        if auto_record_stage == 0.25:
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                action_array[6] = 1
                                auto_record_stage = 0.5

                        if auto_record_stage == 0.5: # lift slightly for random movement to new final position
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                target_pose[2] = 0.09
                                auto_record_stage = 1
                        ...
                        if auto_record_stage == 2: # sample and carry out random rotation
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL: # sampled  position reached
                                ...
                                auto_record_target = current_pose.copy()
                                auto_record_target[2] = 0.08 # target should be below when object touches base
                        ...
                        if auto_record_stage == 6: # move to starting devaited position
                        ...
                        if auto_record_stage == 10: # close gripper
                            if np.linalg.norm(current_pose[:3] - target_pose[:3]) < POS_REACHED_TOL:
                                action_array[6] = 1
