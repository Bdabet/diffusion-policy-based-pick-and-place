...
DEFAULT_OBS_KEY_MAP = {
    ...
    #gripper 
    'OpenDigitalOutState' : 'opening_gripper_output',
    'ClosedDigitalOutState' : 'closing_gripper_output',
...
class RealEnv:
    ...
    def exec_actions(self, 
        ...
        # schedule waypoints
        for i in range(len(new_robot_actions)):
            self.robot.schedule_waypoint(
                pose=new_robot_actions[i],
                target_time=new_timestamps[i]
            )

        for action in new_gripper_actions:
            if action == 1:
                self.robot.gripper_command(True)
                #send close command
